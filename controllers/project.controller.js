...
class ProjectController {
  ...
  async editProject(request, response) {
    try {
      ...
      const updateProj = await this.pool.query(`UPDATE proj SET name ='${name}', description='${description}', due_date='${fomattedDueDate}' WHERE id=${projId}`);
      ...
      slicedArray.forEach(async (chunk) => {
        ...
        if (taskId === '') {
          ...
          const insertUserTask = await this.pool.query(`INSERT INTO user_tasks (user_id, task_id) VALUES ('${receiptId}', '${newTaskId}') RETURNING id`);
          const insertProjTasks = await this.pool.query(`INSERT INTO proj_tasks (proj_id, task_id) VALUES (${projId}, ${newTaskId}) RETURNING id`);
          const insertMessages = await this.pool.query(`INSERT INTO messages (send_to, task_id, accept) VALUES ('${receiptId}', '${newTaskId}', 'pending') RETURNING id`);
        } else {
          const updateTask = await this.pool.query(`UPDATE tasks SET name ='${taskName}', due_date='${formattedTDueDate}' WHERE id=${taskId}`);
          const updateUserTasks = await this.pool.query(`UPDATE user_tasks SET user_id ='${receiptId}' WHERE id='${taskId}'`);
          const updateProjTasks = await this.pool.query(`UPDATE proj_tasks SET proj_id ='${projId}' WHERE id=${taskId}`);
        }
      });
  ...
  async deleteProject(request, response) {
    try {
      ...
      tasks.forEach(async (task) => {
        ...
        const deleteTasks = await this.pool.query(`DELETE FROM tasks WHERE id=${taskId}`);
        const deleteUserTasks = await this.pool.query(`DELETE FROM user_tasks WHERE task_id=${taskId}`);
        const deleteMessages = await this.pool.query(`DELETE FROM messages WHERE task_id=${taskId}`);
      });
      const deleteProj = await this.pool.query(`DELETE FROM proj WHERE id=${projId}`);
      const deleteProjTasks = await this.pool.query(`DELETE FROM proj_tasks WHERE proj_id=${projId}`);
  ...
  async addProject(request, response) {
    try {
      ...
      slicedArray.forEach(async (chunk, index) => {
        ...
        const insertUserTasks = await this.pool.query('INSERT INTO user_tasks (user_id, task_id) VALUES ($1, $2)', [receiptId, taskId]);
        const insertProjTasks = await this.pool.query('INSERT INTO proj_tasks (proj_id, task_id) VALUES ($1, $2)', [projId, taskId]);
        const insertMessages = await this.pool.query('INSERT INTO messages (send_to, task_id, accept) VALUES ($1, $2, $3)', [receiptId, taskId, 'pending']);
      });
      response.redirect('/projects');
    } catch (err) {
      console.error(err);
